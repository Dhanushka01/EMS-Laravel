...
class MnrceAdminDivisionAjaxController extends Controller
{
    function getDivisionByEpfId(Request $req) {            
        $epf=$req['epf'];
        //$epf=91;
                        
        $div1= Mnrce_admin_emp_division::where('epf_id',$epf)
                                    ->where('resDate',NULL)
                                    ->count();
    if($div1==0) {
        ...
        $div= Mnrce_admin_emp_division::where('epf_id',$epf)
                                    ->where('resDate',NULL)
